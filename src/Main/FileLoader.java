...
<TD CLASS="dddefault">see class details</TD>
	...
	public static void main(String[] args) {
		writeAllCoursesFile();
		//new GUI();
		//printDarsSection(DARSInfo.CS_3000);
	}
	
	public static void printDarsSection(final DARSInfo.CLASS[] darsClass) {
		try {		
			...
			M: while ((input = br.readLine()) != null) {			
				if(input.startsWith(VAR.CLASSKEY)) {
					...
					for (DARSInfo.CLASS curClass : darsClass) {
	...
	public static void writeAllCoursesFile() {
		try {		
			File file = new File("bwskfcls.P_GetCrse.htm");	
			final BufferedReader br = new BufferedReader(new FileReader(file));
			String input = null;
